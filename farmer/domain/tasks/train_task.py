...
class TrainTask:
    ...
    def _do_generate_batch_task(self, training_set, validation_set):
        ...
        if self.config.task == ncc.tasks.Task.CLASSIFICATION:
            ...
            train_set = ncc.generator.pytorchClassificationDataset(**sequence_args)
            valid_set = ncc.generator.pytorchClassificationDataset(**sequence_args)
            ...
            if self.config.task == ncc.tasks.Task.SEMANTIC_SEGMENTATION:
                ...
                if self.config.input_data_type == "video":
                    ...
                    if self.config.framework == "tensorflow"
                        ...
                        batch_model_path = os.path.join(
                            self.config.model_path, "batch_model.pth"
                        )
    ...
    def _do_model_optimization_task(
        ...
        if self.config.framework == "tensorflow":
            ...
            # class_weights
            class_weights = torch.tensor(class_weights = self.config.train_params.classification_class_weight).cuda()
            # Loss function
            loss_tr = nn.CrossEntropyLoss(weights=class_weights)
            loss_fn = nn.CrossEntropyLoss(weights=class_weights)
            # scaler
            scaler = GradScaler()
            # optimizer
            if self.config.train_params.optimizer == "adam":
                optimizer = torch.optim.Adam(model.parameters(), lr=self.config.train_params.learning_rate, weight_decay=self.train_params.weight_decay)
            # scheduler
            pass
            ...
            try:
                ...
                for epoch in range(self.config.epochs):
                    train_one_epoch(epoch, model, loss_tr, optimizer, device)
